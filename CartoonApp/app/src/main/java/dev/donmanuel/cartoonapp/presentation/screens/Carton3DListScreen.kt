...
@Composable
fun Carton3DListScreen(
    viewModel: CartoonViewModel = hiltViewModel(),
    modifier: Modifier
) {
    val cartoons3D = viewModel.cartoons3D.collectAsState().value
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            items(cartoons3D) { cartoon ->
                CartoonItem(
                    cartoon = cartoon,
                    isFavorite = favorites.any { it.id == cartoon.id },
                    onFavoriteClick = { viewModel.toggleFavorite(cartoon) },
                    onClick = {
